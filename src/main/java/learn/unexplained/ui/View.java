...
public class View {
    ...
    public EncounterType printAllOfType() {
        //could just call read type in view but decided to leave it private
        return readType();
    }
    ...
    public Encounter update(List<Encounter> encounters) {
        ...
        encounter = findEncounter(encounters);
        if (encounter != null) {
            update(encounter);
        }
        return encounter;
    ...
    public Encounter update(Encounter encounter) {
        ...
        String occurrences = readString("Occurrences: ");
        if (occurrences.trim().length() > 0){
            encounter.setOccurrences(Integer.parseInt(occurrences));
        }
    ...
    public Encounter findEncounter(List<Encounter> encounters) {
        ...
        int encounterId = readInt("EncounterId: ");
        for (Encounter e : encounters) {
    ...
    public void displayResult(EncounterResult result) {
        if (result.isSuccess()) {
            printHeader("Success");
        } else {
            printHeader("Err: ");
